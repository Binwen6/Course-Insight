...
1. 计算出分类误差，并根据该分类误差对样本个体的权重进行调整，并为该弱分类器记录一个分类器权重
1. 用调整权重后的样本全体再次训练一个弱分类器，并再次进行步骤2
1. 迭代步骤2和3
1. 对所有训练完成的弱分类器按照分类器权重表进行加权得到一个强分类器
...
|--|--|
|不用先验地设置聚类数目 | |
| 对数据集的凹凸性不做限制|要求样本集密度均匀且样本点间距不要太大|
|能够自发地发现异常点，因此聚类结果不会受异常点干扰 | 聚类收敛时间较长|
| 不具有初值依赖性| 需要对$\epsilon$和$MinPTs$联合调参，对调参水平要求较高|
