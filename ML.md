...
- 有向图(a.k.a. 时间关联的*贝叶斯网络*)
- 无向图 (a.k.a. 空间或上下文关联的*MRF-马尔可夫随机场*)
- 团
- 最大团
- 势函数
- 条件独立性
...
- 还是知道骰子有几种（隐含状态数量），每种骰子是什么（转换概率），根据掷骰子掷出的结果（可见状态链），我想知道掷出这个**结果的概率**。
- 已知骰子有几种（隐含状态数量），每种骰子是什么（转换概率），根据掷骰子掷出的结果（可见状态链），我想知道每次掷出来的都是哪种骰子（**隐含状态链**）。
...
初始化
**递推公式**：
$$a_t=p(y_1,y_2,\cdot\cdot\cdot,y_t,s_t)=p(y_t|s_t)\sum_{s_{t-1}}a_{t-1}p(s_t|s_{t-1})\\ p(y_t|s_t)为由状态j生成观测y_t的概率q_{jy_t}\\ p(s_t|s_{t-1}为由状态i转移为状态j的概率p_{ij}$$
算法复杂度：直接求和算法$O(2n|S|^n)$->前向算法$O(n|S|^2)$
